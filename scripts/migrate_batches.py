...
Splits the full migration into 5 batches (20% each) in separate files.
Ensures no products are missed across all batches.
"""
...
def split_source_csv(source_csv_path: str, num_batches: int = 5) -> list:
    """
    ...
    def row_has_price(row: pd.Series) -> bool:
        for field in ['Regular price', 'Sale price']:
            if field in row and pd.notna(row[field]):
                try:
                    price = float(str(row[field]).replace('$', '').replace(',', '').replace('₹', '').strip())
                    if price > 0:
                        return True
                except Exception:
                    continue
    ...
    def row_has_image(row: pd.Series) -> bool:
        ...
        return img_str != '' and img_str.lower() != 'nan'
    ...
    for group_id, group_df in df.groupby('__ProductGroupID', sort=False):
        ...
        if has_price and has_image:
            ...
            invalid_groups.append((group_id, has_price, has_image))
    
    valid_groups.sort(key=lambda item: item[0])
    logger.info(f"Total product groups: {len(valid_groups) + len(invalid_groups)}")
    logger.info(f"Valid product groups (price + image): {len(valid_groups)}")
    logger.info(f"Skipped product groups (missing data): {len(invalid_groups)}")
    
    # Partition valid groups evenly across batches while preserving order
    batch_dfs = []
    valid_row_count = sum(len(group_df) for _, _, group_df in valid_groups)
    invalid_row_count = total_rows - valid_row_count
    ...
    if total_batch_rows != valid_row_count:
        logger.error(f"ERROR: Row count mismatch! Valid rows: {valid_row_count}, Batches: {total_batch_rows}")
        raise ValueError(f"Row count mismatch: {valid_row_count} != {total_batch_rows}")
    
    logger.info(f"✓ Distributed {valid_row_count} valid rows across {num_batches} batches without splitting products")
    if invalid_groups:
        logger.info(f"ℹ️ Skipped {invalid_row_count} rows across {len(invalid_groups)} invalid product groups (missing price/image)")
    return batch_dfs
...
def save_batch_csv(batch_df: pd.DataFrame, batch_num: int, temp_dir: Path) -> str:
    ...
    Args:
        ...
        temp_dir: Directory to save temporary CSV files
...
def migrate_batch(
    ...
    Args:
        batch_source_csv: Path to batch source CSV
        batch_num: Batch number (1-based)
        shopify_template: Path to Shopify template CSV
    ...
    # Count rows in batch source
    batch_df = pd.read_csv(batch_source_csv, low_memory=False)
    batch_size = len(batch_df)
    
    print(Fore.CYAN + f"\n{'='*80}")
    print(Fore.CYAN + f"BATCH {batch_num}/{total_batches} - {batch_size} Products")
...
def verify_all_batches(output_files: list) -> dict:
    ...
    Args:
        output_files: List of output CSV file paths
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Migrate products in 5 batches (20% each) to separate files'
    )
    parser.add_argument(
        '--source',
        type=str,
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        help='Output directory for batch CSV files'
    )
    parser.add_argument(
        '--mapping',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Number of batches to create (default: 5)'
    )
    parser.add_argument(
        '--batch-only',
        type=int,
        help='Run only a specific batch number (1-based). If not provided, runs all batches.'
    )
    ...
    print(Fore.CYAN + "=" * 80)
    print(Fore.CYAN + f"SHOPIFY PRODUCT MIGRATION - {num_batches} BATCHES")
    print(Fore.CYAN + "=" * 80)
    ...
    try:
        ...
        batches = split_source_csv(source_csv, num_batches)
        ...
        for i, batch_df in enumerate(batches, 1):
            ...
            if (i - 1) not in batches_to_run:
                ...
                output_file = output_path / f"shopify_products_batch_{i}_of_{num_batches}.csv"
