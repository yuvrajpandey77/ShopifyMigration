#!/usr/bin/env python3
"""
Full Migration Script - 20 Batches
Migrates ALL products (even with missing images/prices/descriptions) in 20 batches.
"""
...
def split_source_csv_all_products(source_csv_path: str, num_batches: int = 20) -> list:
    ...
    INCLUDES ALL PRODUCTS (even without price/image/description).
    ...
    logger.info(f"Total product groups: {len(valid_groups):,}")
    logger.info(f"Including ALL products (no filtering by price/image)")
    ...
    logger.info(f"✓ Distributed {total_batch_rows:,} rows across {num_batches} batches without splitting products")
    return batch_dfs
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help='Number of batches to create (default: 20)'
    )
    parser.add_argument(
        '--batch-only',
        type=int,
    ...
    print(Fore.YELLOW + "NOTE: ALL products will be migrated (including those with missing images/prices/descriptions)")
    print(Fore.CYAN + "=" * 80)
    print()
